...
class HomeController extends Controller
{
	...
	{
		$result = DB::table('sheep')->inRandomOrder()->where('active', 1)->first();
		$id = $result->id;
		$paddock = $result->paddock;

		$result = DB::table('sheep')->where('id',$id)->update(['active' => 0]);

		$msg = json_encode(['killed_ship_id' => $id, 'paddock' => $paddock ]);
